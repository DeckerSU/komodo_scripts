<?php
// notaries.json generator for Insight Explorers (c) Decker 2018-2020

$Notaries_elected_S1_mainnet = Array(
...
//$n = array_intersect($Notaries_elected1, $notaries_elected);
$n1 = array_flip($Notaries_elected_S1_mainnet);
$n2 = array_flip($Notaries_elected_S2_mainnet);
$n3 = array_flip($Notaries_elected_S3_mainnet);
$n4 = array_flip($Notaries_elected_S4_mainnet);

foreach($n1 as $key => &$value) $value .= " (s1)";
foreach($n2 as $key => &$value) $value .= " (s2)";
foreach($n3 as $key => &$value) $value .= " (s3)";

$n = array_merge($n1, $n2, $n3, $n4);
...
foreach ($n as $key => $value) {
	//echo $key . " - "."21".$value."ac"."\r\n";
	//$res = json_decode(shell_exec("./komodo-cli decodescript \"21".$value."ac\""));
	//var_dump($res->addresses[0]);
	//echo '"'. $res->addresses[0] . '" => "' . $key . '"'.",\r\n";
	echo '  {
    "notaryName": "'.$key.'",
    "pubkey": [
      "'.$value.'"
