...
foreach ($Notaries_elected_S8_3rdparty as $key => $value) {
    $bitcoinECDSA->setNetworkPrefix(sprintf("%02X", 0));
    // $btc_address = $bitcoinECDSA->getUncompressedAddress(true, $value);
    // echo $value . ' -> ' . $btc_address . PHP_EOL;

    $pubkey = $bitcoinECDSA->getPubKey($bitcoinECDSA->getPubKeyPointsWithDerPubKey($value));

    $ripemd160 = $bitcoinECDSA->hash160(pack("H*",$pubkey));
    ...
    echo $key . ' -> ' . $bech32_address . PHP_EOL;
}
?>