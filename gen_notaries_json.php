...
$n0 = array_flip($notaries_elected);
$n1 = array_flip($Notaries_elected1);
$n2 = array_flip($Notaries_elected_S3_mainnet);

foreach($n0 as $key => &$value) $value .= " (s1)";
foreach($n1 as $key => &$value) $value .= " (s2)";
foreach($n2 as $key => &$value) $value .= "";

$n = array_merge($n0, $n1, $n2);
//var_dump($n);
$n = array_flip($n);
//var_dump($n);
...
foreach ($n as $key => $value) {
    ...
    $pos = strpos($key, "_");
    if ($pos !== false) {
	$url = "https://github.com/KomodoPlatform/NotaryNodes/blob/master/notarynodes/".substr($key, 0, $pos)."/README.md";
    }
    */
	//echo $key . " - "."21".$value."ac"."\r\n";
	//$res = json_decode(shell_exec("/home/decker/ssd_m2/komodo_src/src/komodo-cli decodescript \"21".$value."ac\""));
